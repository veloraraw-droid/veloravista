...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error("[invoice-checkout] Stripe session creation failed", { invoiceId: id, error: error instanceof Error ? error.message : String(error) });
    return NextResponse.redirect(new URL(`/client-portal?tab=billing&invoice=${id}&payment=failed`, origin), 303);
  }
}
